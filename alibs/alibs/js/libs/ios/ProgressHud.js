...
(function(cordova) {
	...
	ProgressHud.prototype.setText = function(options, callback) {
		if(!options) options = {};
		var scope = options.scope || null;
		delete options.scope;
...
var progressHud;
var progressHudState = false;
function showHud(text){
    if(!progressHud) progressHud = window.plugins.progressHud;
    progressHud.show({mode: "indeterminate", labelText: text});
	progressHudState = true;
}

function hideHud(){
	if(progressHudState){
		progressHud.hide();
		progressHudState = false;
	}
}

function setHudText(text){
	if(progressHudState)
		progressHud.setText({text: text});
}