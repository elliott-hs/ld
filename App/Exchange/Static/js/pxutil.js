var Util = (function () {
	...
	obj.prototype = {
		...
		showmsg: function (msg, funok, funcancel, key,code,tid,openid) {
			//1.
		 ...
		 funok:function(code,tid,openid) {
			// body...
			
			Util.ajax({
				method:'post',
				data:'tid='+tid+'&openid='+openid,
				url:'/ldh/exchange.php/Jiyuedu/exchange'},
				function(json){
					if(json.status  == 1 ){
						if(tid == 2){
							$(".num").html(0);
						}
						code.removeClass("exchange")
						code.off("click");
						code.html("兑奖码<br>"+json.key);
					}else{
						Util.showmsg(json.info,undefined,"","","","","");
					}
		...
		showLoading: function () {
			...
			if (!loading) {
				...
				loading.id = "loading";
				loading.innerHTML = '<img src="/ldh/uploads/loading.gif" />';
